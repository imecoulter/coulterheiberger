 ...
 *
 * Two rendering paths were probed and both are unusable here:
 *
 ...
 *     Mono is not installed on this machine and fell back with no error. A card
 *     built that way would ship in the wrong typeface and look fine locally.
 *
 * So the glyphs are outlined out of the committed woff2 with fontTools and
 * emitted as a single `<path>`. librsvg then renders geometry, not type, and has
 * no font to miss — and any character outside the shipped subsets is a hard
 * error here, where in a `<text>` element it would be a silent per-glyph
 * fallback nobody would catch until the card was in someone's timeline.
 *
 ...
 */
function outline(fontPath, text, size, tracking) {
