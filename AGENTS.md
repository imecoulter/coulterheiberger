...
- **The site has one Ground and it is dark.** No light alternative, no `prefers-color-scheme` fallback, no toggle — `npm run check:css` refuses that query in site CSS by name. The one exemption is the favicon SVG, which answers a question about the browser's tab strip rather than about this site. See [ADR-0007](./docs/adr/0007-one-dark-ground.md).
- **WCAG AA**, gated in CI on every page. `--muted` holds ≥ 4.5:1 on `--ground` — it is the colour of 11–12 px text, so the large-text 3:1 exemption does not apply and is refused by name. `--signal` holds ≥ 3:1 and **is never a text colour**: it is the focus outline, and it fails 4.5:1. Both are asserted by `npm run check:css` against the built CSS.
...
- **Spacing is a token, never a number.** `--gap` is the one distance between things (Plate to Plate, Plate to screen edge); everything smaller is `--s1`..`--s5`. `npm run check:css` refuses a raw length in `margin`, `padding` or any `gap` in built site CSS. The scale and the reason for the cap are in [docs/styling.md](./docs/styling.md); the arrangement it serves is [ADR-0008](./docs/adr/0008-the-index-arrangement-and-one-spacing-atom.md).
...
- **The site has one motion device, applied in two places.** **Registration** is the one-time entrance of an element as it first comes into view: `opacity` 0 to 1 with an 8px `translateY`, 760ms `ease-out`, staggered 60ms apart in groups of four, fired once and never re-triggered on scroll back ([ADR-0010](./docs/adr/0010-registration-returns.md)). **The two reveals** are the same move under a pointer, and since [ADR-0011](./docs/adr/0011-the-masthead-is-site-furniture.md) they are one component (`src/components/Meta.astro`) rendered by two objects: a Plate's metadata band on `/`, and the Masthead portrait's `About` label on every route but `/about/ime/`. Each fades its scrim in over 760ms and rising its text 8px into it 60ms behind, out at 480ms ([ADR-0008](./docs/adr/0008-the-index-arrangement-and-one-spacing-atom.md), [ADR-0009](./docs/adr/0009-the-two-reveals-are-timed.md)). Same numbers, deliberately: that is what makes it one device. **The rule is the device, not the count** — an element entering the same way, or a third element revealing a label the same way, is the same decision; anything that moves for another reason needs the amendment below. A navigation is still an ordinary full page load, nothing parallaxes, and nothing scrubs with scroll position.
- **The motion lives in exactly two places and both are load-bearing.** The four custom properties (`--rise`, `--reveal-in`, `--reveal-out`, `--reveal-stagger`) and Registration's shared rules are in `src/styles/base.css`; the two reveals are in `src/pages/index.astro`'s scoped block. **Every `prefers-reduced-motion` query on the site is a `no-preference` gate, never a `reduce` block** — three of them: one in `base.css` for Registration, and two in `index.astro`'s scoped block, one per reveal — the hidden state is declared *inside* the gate so that `reduce` fails toward stillness rather than computing the animated path first. Never invert either one, and never move the untimed `opacity` endpoints inside the reveals' gate. Registration is **not** hover-gated; the reveals are, because a coarse pointer cannot trigger them. **A `transform` has a silent trap**: it becomes the containing block for `.title a::after` / `.about a::after` and collapses the whole-tile hit area, which is why the rise is carried by a `<span>` inside each anchor, and why Registration goes on the `<li class="plate">` and never between it and the `::after`. [docs/motion.md](./docs/motion.md) holds the specs, the byte costs, and the findings that each fail silently. **Read it before writing a `transition`**, and note the order is binding: [docs/design-direction.md](./docs/design-direction.md) is amended first, then a mechanism is chosen to serve it. Never a library first — every candidate was priced in [issue #12](https://github.com/imecoulter/coulterheiberger-com/issues/12).
- The site has exactly one *authored* `<script>`, in `Base.astro`, and it now does three things: warming the hero image on pointer enter, Registration's `IntersectionObserver`, and the Expanded View's `<dialog>`. **The rule is one authored `<script>`, not one behaviour** — adding a second file, an island, or a library means amending [docs/design-direction.md](./docs/design-direction.md) first. The Cloudflare Web Analytics beacon in `Base.astro` is the one script not authored here — committed rather than edge-injected, counted against the 50 KB budget, and governed by [ADR-0004](./docs/adr/0004-measurement-committed-beacon-no-field-cwv.md). Costs and conditions are in [docs/styling.md](./docs/styling.md).
