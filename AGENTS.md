...
- **The site has no motion.** No animation, no transition, no `@keyframes`, no `@view-transition`, and no `prefers-reduced-motion` query. Pages paint in their final state and a navigation is an ordinary full page load. **There are exactly two `:hover` rules**, both on `/` and both the same device: a Plate's metadata band, and the Masthead portrait's `About` label. Each is an `opacity` swap with no duration and no easing ([ADR-0008](./docs/adr/0008-the-index-arrangement-and-one-spacing-atom.md)). **The rule is the instant swap, not the count** — a duration on either, or a third element that is not this same reveal, is motion and needs the amendment below. This is the current direction, not a gap: five interlocking categories were removed in one pass, and [docs/motion.md](./docs/motion.md) holds all of it — the specs, the byte costs, and four findings that each fail silently. **Read it before writing a `transition`**, and note the order is binding: [docs/design-direction.md](./docs/design-direction.md) is amended first, then a mechanism is chosen to serve it. Never a library first — every candidate was priced in [issue #12](https://github.com/imecoulter/coulterheiberger-com/issues/12).
